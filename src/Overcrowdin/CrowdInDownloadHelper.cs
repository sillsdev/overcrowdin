...
namespace Overcrowdin
{
    internal sealed class CrowdInDownloadHelper : CrowdInHelperBase
    {
        ...
        #region Constructor
        private CrowdInDownloadHelper(CrowdinProjectSettings settings, IFileSystem fs, IHttpClientFactory factory = null) : base(settings, fs, factory)
		{
		}
        #endregion

        #region Public methods
        public static CrowdInDownloadHelper Create(CrowdinProjectSettings settings, IFileSystem fs, IHttpClientFactory factory = null)
        {
            return Initialize(settings, fs, factory, (s, f, h) => new CrowdInDownloadHelper(s, f, h));
        }

        public bool DownloadTranslations(string outputPath)
        {
            try
            {
                Task<bool> task = DownloadInternal(outputPath);
                task.Wait();
                return task.Result;
            }
            catch (Exception e)
