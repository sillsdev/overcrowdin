...
namespace Overcrowdin
{
	public static class CommandUtilities
	{
		...
		private static void AddSuperFolders(ISet<string> folders)
		{
			foreach (var folder in folders.ToArray())
			{
				var superFolder = SuperFolder(folder);
				while (!string.IsNullOrEmpty(superFolder))
				{
					folders.Add(superFolder);
					superFolder = SuperFolder(superFolder);
				}
			}
		}

		private static string SuperFolder(string path)
		{
